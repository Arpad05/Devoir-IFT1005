...
function submitForm(){
    ...
    submitButton.addEventListener('click', function(event){
        ...
        var transfert={id:ID, name:listJSON[ID-1].name, quantity:qty, price:listJSON[ID-1].price}
        var myJsonObj=JSON.stringify(transfert);
        window.localStorage.setItem(name, myJsonObj);
        window.location.href='/product.html\?id='+ encodeURI(ID);

        window.localStorage.setItem('itemsInBasket', 'true');
        var nbItems = parseInt(window.localStorage.getItem('nbItems'));
        if (nbItems){nbItems = nbItems + parseInt(qty);} else{nbItems = qty;}
